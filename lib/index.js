...
function renderResponsive(md, tokens, idx, options, env, self, patterns) {
  // normal fields
  const srcfilename = tokens[idx].src;
  const src = ' src="' + srcfilename + '"';
  const filename = stripQueryString(srcfilename);
  var title = '';
  if (tokens[idx].title) {
  ...
  if ((patternId = matchPattern(filename, patterns)) >= 0) {
    ...
    for (i = 0; i < rules.length; i++) {
      ...
      // netlify
      if (rules[i].nf_resize && rules[i].width) {
        srcset += '?nf_resize=' + rules[i].nf_resize + '&w=' + rules[i].width;
      }
